...
public class CreateSpecialPowersPool : MonoBehaviour
{
    public static CreateSpecialPowersPool SharedInstance;
    ...
    void Awake()
    {
        ...
        for (int z = 0; z < objectToPool.Length; z++)
        {
            ...
            for (int i = 0; i < amountToPool[z]; i++)
            {
                ...
                tmp.GetComponent<SpecialPowerEngine>().listID = z;
                tmp.SetActive(false);
                tempPool.Add(tmp);
            }
    ...
    public GameObject GetPooledObject(int poolNumber)
    {
        ...
        if (!specialPowersList.specialPowersList[poolNumber].isUnlocked)
        {
            return null;
        }
