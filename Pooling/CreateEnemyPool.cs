...
{
    public static CreateEnemyPool SharedInstance;
    ...
    void Awake()
    {
        ...
        for (int z = 0; z < objectToPool.Length; z++)
        {
            ...
            for (int i = 0; i < amountToPool[z]; i++)
            {
                ...
                tmp.GetComponent<TopDownEnemyEngine>().firstDisable = true;
                tmp.GetComponent<TopDownEnemyEngine>().listID = z;
                tmp.SetActive(false);
                tempPool.Add(tmp);
            }
    ...
    public GameObject FillPoll(int poolNumber)
    {
        ...
        tmp.GetComponent<TopDownEnemyEngine>().firstDisable = false;
        tempPool.Add(tmp);
